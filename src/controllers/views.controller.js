...
const loginUser = async (req, res) => {
  ...
  res.status(200).send({
    status: 'success',
    token: token,
    destiny: destiny,  // Agrega la propiedad destiny a la respuesta JSON
...
const logoutUser = async (req, res) => {
  ...
  req.session.destroy((error) => {
    if (error) {
      return res
        .status(500)
        .json({ error: `La sesión no se ha cerrado correctamente` });
    }
  ...
  try {
    ...
    if (!user)
      return res.render("resetPassword", {
        error: `El usuario con el mail ${email} no existe`,
      });
...
const userEdit = async (req, res) => {
  ...
  try {
    ...
    res.render('editUser', { showNav: true, user });
  } catch (error) {
      logger.error(error);
...
const userDelete = async (req, res) => {
  ...
  try {
      ...
      res.render('deleteUser', { showNav: true, user });
  } catch (error) {
      logger.error(error);
      res.status(500).send('Error interno del servidor');
  }
...
const productDetail = async (req, res) => {
  try {
    ...
    if (!token) {
      // Manejar caso en que no hay token
      return res.status(401).send('Unauthorized: No token provided');
    }

    const dataToken = verifyToken(token);

    if (!dataToken || !dataToken.user || !dataToken.user.email) {
      // Manejar caso en que el token no contiene la información esperada
    ...
    if (!user) {
      // Manejar caso en que no se encuentra el usuario
    ...
    if (!cart) {
      // Manejar caso en que no se encuentra el carrito
